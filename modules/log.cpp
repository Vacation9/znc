...
void CLogMod::PutLog(const CString& sLine, const CString& sWindow /*= "Status"*/)
{
	...
	sPath.Replace("$WINDOW", sWindow.Replace_n("/", "-").Replace_n("\\", "-").MakeLower());
	...
	{
		LogFile.Write(CUtils::FormatTime(curtime, "[%m/%d/%Y][%H:%M:%S] ", m_pUser->GetTimezone()) + (m_bSanitize ? sLine.StripControls_n() : sLine) + "\n");
	} else
...
GLOBALMODULEDEFS(CLogMod, "Write IRC logs")
